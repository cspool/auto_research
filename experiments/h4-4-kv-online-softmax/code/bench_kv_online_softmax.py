#!/usr/bin/env python3
"""H4.4 benchmark: multi-block online-softmax sparse attention."""
...
def reduce_sparse_ssv_kernel(
    ...
    tl.store(out_ptr + v_cols, acc / denom, mask=v_cols < value_dim)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--variants",
        default="64,64,64,4;128,64,64,4;256,64,64,4;128,128,64,4;256,128,64,4;128,64,128,4;128,64,64,8",
    )
...
class OnlineSsvWorkload:
    def __init__(
        ...
        self._partial_cache: dict[tuple[int, int], tuple[torch.Tensor, torch.Tensor, torch.Tensor]] = {}
    ...
    def partial_buffers(self, block_n: int, block_v: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        key = (block_n, block_v)
        ...
        self._partial_cache[key] = (partial_m, partial_l, partial_o)
    ...
    def triton_online(self, block_n: int, block_d: int, block_v: int, warps: int) -> None:
        partial_m, partial_l, partial_o = self.partial_buffers(block_n, block_v)
        num_n_blocks = triton.cdiv(self.selected_tokens, block_n)
        num_v_blocks = triton.cdiv(self.value_dim, block_v)
        grid_partial = (num_n_blocks, num_v_blocks)
        partial_sparse_ssv_kernel[grid_partial](
            self.k_cache,
            self.v_cache,
            self.q,
            self.indices,
        ...
        reduce_sparse_ssv_kernel[(num_v_blocks,)](
            partial_m,
            partial_l,
            partial_o,
            self.out_triton,
    ...
    def validate_variant(self, block_n: int, block_d: int, block_v: int, warps: int) -> dict:
        self.torch_flat()
        self.triton_online(block_n, block_d, block_v, warps)
        torch.cuda.synchronize()
...
def run(args: argparse.Namespace) -> dict:
    ...
    for block_n, block_d, block_v, warps in variants:
        elapsed = cuda_time(
            lambda bn=block_n, bd=block_d, bv=block_v, nw=warps: workload.triton_online(bn, bd, bv, nw),
            args.warmup,
            args.iters,
        )
        ...
        variant_results.append(
            {
                ...
                "triton_online_ms": elapsed,
                "triton_vs_flat_speedup": flat_total_ms / elapsed,
                "validation": validation,
            }
        )

    best = min(variant_results, key=lambda item: item["triton_online_ms"])
    return {
        "status": "ok",
        "order_mode": args.order_mode,
        "segment_counts": segment_counts,
        ...
        "best_triton_online_ms": best["triton_online_ms"],
        "best_triton_vs_flat_speedup": flat_total_ms / best["triton_online_ms"],
        "best_variant": {
    ...
    with (out_dir / "summary.csv").open("w", newline="", encoding="utf-8") as handle:
        ...
        writer.writerow(
            [
                ...
                result.get("best_triton_online_ms"),
